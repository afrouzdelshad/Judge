...
Reads task4{a,b,c}_data.csv (same "time" + one "(x, y)" column per label
layout as task1/task2/task3, but with exactly two labels, A and B) and
...
Usage
-----
    python task4.py --variant a --model claude-opus-4-8
    python task4.py --variant b --model claude-opus-4-8
    python task4.py --variant c --model claude-opus-4-8
...
DATA_FILES = {"a": "task4a_data.csv", "b": "task4b_data.csv", "c": "task4c_data.csv"}
KEY_FILES = {"a": "task4a_key.csv", "b": "task4b_key.csv", "c": "task4c_key.csv"}
OUTDIRS = {"a": "task4a_runs", "b": "task4b_runs", "c": "task4c_runs"}


def load_key(path):
    """Return the set of true claim numbers from a task4{a,b,c}_key.csv, or None.
...
def run_plain_batch(model, labels, times, columns, n, use_batch=False):
    ...
    system_prompt = TASK4_PLAIN_PROMPT
